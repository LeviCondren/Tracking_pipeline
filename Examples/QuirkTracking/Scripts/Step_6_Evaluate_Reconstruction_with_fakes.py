...
def evaluate(config_file=CONFIG):
    ...
    #print("unique PIDs, counts",np.unique(particles_data["PID"], return_counts=True)) # There is no signal PID, but there is a nan
    ...
    if 15 in np.unique(particles_data["PID"]):
        ...
        print("reconstruct_data_signal", reconstruct_data_signal)
